...
Now using ConfigCore via `load_config`.
"""
...
@patch("minecraft.deploy_pack.cfg.load_config")
...
def test_main_server_mode(
    ...
    mock_config.as_dict.return_value = {"sync_root": str(sync)}  # for debug print
...
def test_main_client_mode(
    ...
    mock_config.as_dict.return_value = {"sync_root": str(sync)}
    mock_load_config.return_value = mock_config
...
def test_main_client_mode_missing_instance_name():
    ...
    with patch("minecraft.deploy_pack.cfg.load_config") as mock_load_config:
        mock_config = MagicMock()
        mock_config.get.side_effect = lambda key, default=None: (
