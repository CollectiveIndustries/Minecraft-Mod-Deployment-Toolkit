...
def test_get_mod_file_url(mock_api):
    ...
    url, filename = mod_puller.get_mod_file_url(mock_client, "testmod", "1.0.0", "1.20.1")
...
def test_get_mod_file_url_no_version_match():
    ...
    with patch('minecraft.mod_puller.CurseForgeAPI') as mock_api:
        ...
        with pytest.raises(ValueError, match="No file found for 'testmod' version '1.0.0' on MC 1.20.1"):
...
def test_main_curseforge_mod(mock_download, mock_api, mock_load_manifest,
                             mock_get_logger, mock_setup_logging,
                             mock_ConfigManager, mock_find_config, tmp_path):
    ...
    mock_client = mock_api.return_value
    mock_get_url = MagicMock(return_value=("http://example.com/create.jar", "create.jar"))
    ...
    mock_download.assert_called_once_with("http://example.com/create.jar", expected_path)
...
@patch('minecraft.mod_puller.get_logger')
@patch('minecraft.mod_puller.load_manifest')
def test_main_local_file(mock_load_manifest, mock_get_logger, mock_setup_logging,
                         mock_ConfigManager, mock_find_config, tmp_path):
...
@patch('minecraft.mod_puller.get_logger')
@patch('minecraft.mod_puller.load_manifest')
def test_main_disabled_mod(mock_load_manifest, mock_get_logger, mock_setup_logging,
                           mock_ConfigManager, mock_find_config, tmp_path):
...
@patch('minecraft.mod_puller.get_logger')
@patch('minecraft.mod_puller.load_manifest')
def test_main_no_api_key(mock_load_manifest, mock_get_logger, mock_setup_logging,
                         mock_ConfigManager, mock_find_config):
