...
def _client_source_dir(config: DeploymentConfig) -> Path | None:
    ...
    if not isinstance(client_sub, str) or not client_sub:
        return None
    return config.sync_root / client_sub


def _resource_pack_mapping(config: DeploymentConfig) -> str | None:
...
def _validate_and_plan_publishes(
    config: DeploymentConfig, partition: list[str], logger: Any
    ...
    for member, rp in members_with_pack:
        ...
        try:
            dest_dir = resolve_resource_pack_dest(dest_value, config.www_dir)
        except ConfigError as exc:
            return (pending, no_pack, str(exc), member)
        ...
        pending.append(_PendingPublish(member=member, rp=rp, source=source, sha1=sha1, destination=destination, url=url, target=target))
    return (pending, no_pack, None, None)


def _needs_publish(destination: Path, source_sha1: str) -> bool:
    """§4.4: publish when destination missing OR SHA-1 differs.

    SHA-1 comparison is case-insensitive - the computed value is already
    lowercase, and so is what we produce. This guard is for a destination
    that was written by something else.
    """
    if not destination.is_file():
        return True
    try:
        return compute_sha1(destination).lower() != source_sha1.lower()
    except OSError:
        return True


def deploy_resource_pack_scope(config: DeploymentConfig, plan: PreflightPlan, protect_patterns: list[str], logger: Any = None) -> ResourcePackScopeResult:
    ...
    if not pending:
        if logger is not None:
            logger.info("resource-pack scope: no packs configured for partition")
        return result
    for p in pending:
        try:
            needed = _needs_publish(p.destination, p.sha1)
        ...
        try:
            ...
            return result
        result.publish_results.append(
            PublishResult(member=p.member, filename=p.rp.filename, source=p.source, destination=p.destination, sha1=p.sha1, published=True)
        )
        if logger is not None:
    ...
    for p in pending:
        ...
        result.properties_results.append(PropertiesWriteResult(member=p.member, path=props_path, changes=changes, wrote=diff.any))
        if logger is not None and diff.any:
